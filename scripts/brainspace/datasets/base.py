...
def load_hcp_parcellation(name, scale=400, join=False):
    """ Load parcellation for conte69.
    ...
    root_pth = os.path.dirname(__file__)
    fname = '{name}_{np}_hcp360.csv'.format(name=name, np=scale)
    ipth = os.path.join(root_pth, 'parcellations', fname)
    x = np.loadtxt(ipth, dtype=np.int)
    if join:
        return x
...
def hcp360(with_normals=True, join=False):

    root_pth = os.path.dirname(__file__)

    fname = 'S1200.{}.midthickness_MSMAll.32k_fs_LR.surf.gii'
