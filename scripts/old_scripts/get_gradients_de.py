...
def get_grad_df(side,matrix):
    ...
    gradient_df = pd.DataFrame({side+'_grad_1': gm[:,0], side+'_grad_2': gm[:,1], side+'_grad_3': gm[:,2], side+'_grad_4': gm[:,3]})
...
def grad_calculater(bold_file):
    ...
    gradient_L_df,gm_L = get_grad_df('L',sim_matrix_L)
    gradient_R_df,gm_R = get_grad_df('R',sim_matrix_R)

    gradient_ctx_L_df,gm_ctx_L = get_grad_df('ctx_L',sliced_ctx_L_matrix)
    gradient_ctx_R_df,gm_ctx_R = get_grad_df('ctx_R',sliced_ctx_R_matrix)

    np.save(snakemake.output.gm_L_emb, gm_L)
    np.save(snakemake.output.gm_R_emb, gm_R)
    np.save(snakemake.output.gm_ctx_L_emb, gm_ctx_L)
    np.save(snakemake.output.gm_ctx_R_emb, gm_ctx_R)
